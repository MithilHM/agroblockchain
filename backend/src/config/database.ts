  ...
  try {
    ...
    if (error) {
      if (error.code === 'PGRST116') {
        ...
        logger.warn('⚠️  Database tables not found. Please create them using the Supabase dashboard.');
        logger.info(`
...
CREATE INDEX idx_users_role ON users(role);
        ...
        logger.info('⚡ Server will continue running. User registration will fail until tables are created.');
        return;
      } else {
        throw error;
      }
    ...
    if (process.env.NODE_ENV !== 'production') {
      logger.warn('⚠️  Continuing in development mode despite database issues...');
      return;
    }
    process.exit(1);
  }
