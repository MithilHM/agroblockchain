...
export class WalletController {
  ...
  async getWalletDetails(req: Request, res: Response): Promise<void> {
    try {
      ...
      const totalOutgoing = outgoingTransfers.reduce((sum, t) => 
        sum + parseFloat(t.price_transferred || '0'), 0);
      const totalIncoming = incomingTransfers.reduce((sum, t) => 
        sum + parseFloat(t.price_transferred || '0'), 0);
      const currentInventoryValue = currentBatches.reduce((sum, b) => 
        sum + (parseFloat(b.price_per_unit || '0') * parseFloat(b.quantity || '0')), 0);
  ...
  async updateWalletAddress(req: Request, res: Response): Promise<void> {
    try {
      ...
      logger.info(`User ${userId} updated wallet address to ${wallet_address}`);
    } catch (error) {
      logger.error('Update wallet address error:', error);
      res.status(500).json({
        success: false,
  ...
  async getEarningsSummary(req: Request, res: Response): Promise<void> {
    try {
      ...
      if (salesError) {
        logger.error('Get earnings summary error:', salesError);
        res.status(500).json({
          success: false,
      ...
      const totalEarnings = sales?.reduce((sum, sale) => 
        sum + parseFloat(sale.price_transferred || '0'), 0) || 0;
