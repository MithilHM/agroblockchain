...
exports.distributorController = exports.DistributorController = void 0;
const supabase_1 = require("../config/supabase");
const logger_1 = require("../utils/logger");
const notificationController_1 = require("./notificationController");
class DistributorController {
    // Get available distributors for farmers to select from
    async getAvailableDistributors(req, res) {
        try {
            ...
            const { data: distributors, error } = await supabase_1.supabaseAdmin
                .from('users')
                .select(`
          id,
          name,
        ...
        `)
            ...
            if (error) {
                logger_1.logger.error('Get distributors error:', error);
                res.status(500).json({
                    success: false,
            ...
            const distributorStats = await Promise.all(distributors.map(async (distributor) => {
                ...
                return {
                    ...
                    statistics: {
                        ...
                        current_inventory_value: currentBatches.reduce((sum, batch) => sum + (parseFloat(batch.price_per_unit || '0') * parseFloat(batch.quantity || '0')), 0),
            ...
            if (product_type) {
                filteredDistributors = distributorStats.filter(dist => dist.specialties.includes(product_type.toString().toLowerCase()));
            }
            // Sort by rating and experience
            filteredDistributors.sort((a, b) => (b.statistics.average_rating - a.statistics.average_rating) ||
            ...
            logger_1.logger.error('Get available distributors error:', error);
            res.status(500).json({
                success: false,
                message: 'Failed to retrieve distributors'
            });
    ...
    async createBatchOffer(req, res) {
        try {
            const userId = req.user?.userId;
            const userRole = req.user?.role;
            ...
            const offerPromises = distributor_ids.map(async (distributorId) => {
            ...
            const notificationPromises = distributor_ids.map(async (distributorId) => {
                return notificationController_1.NotificationController.createNotification({
            ...
            logger_1.logger.info(`Farmer ${userId} created batch offers for batch ${batch_id} to ${distributor_ids.length} distributors`);
        }
        catch (error) {
            logger_1.logger.error('Create batch offer error:', error);
            res.status(500).json({
                success: false,
    ...
    async getDistributorOffers(req, res) {
        try {
            const userId = req.user?.userId;
            const userRole = req.user?.role;
        ...
        `)
            ...
            if (error) {
                logger_1.logger.error('Get distributor offers error:', error);
                res.status(500).json({
                    success: false,
            ...
            const processedOffers = offers?.map(offer => ({
                ...offer,
                time_remaining_hours: Math.max(0, Math.floor((new Date(offer.expiry_time).getTime() - Date.now()) / (1000 * 60 * 60))),
    ...
    async respondToOffer(req, res) {
        try {
            ...
            // Get offer details
            const { data: offer, error: offerError } = await supabase_1.supabaseAdmin
                .from('batch_offers')
                .select(`
          *,
        ...
        `)
            ...
            const { error: updateError } = await supabase_1.supabaseAdmin
                ...
                .eq('id', offer.id);
            if (updateError) {
                logger_1.logger.error('Respond to offer error:', updateError);
                res.status(500).json({
                    success: false,
            ...
            logger_1.logger.info(`Distributor ${userId} ${action}ed offer ${offerId}`);
        }
        catch (error) {
            logger_1.logger.error('Respond to offer error:', error);
            res.status(500).json({
                success: false,
                message: 'Failed to respond to offer'
            });
    ...
    async getInventoryAnalytics(req, res) {
        try {
            ...
            const { data: inventory, error: inventoryError } = await supabase_1.supabaseAdmin
                .from('produce_batches')
                .select('*')
                .eq('current_owner_id', userId)
                .in('status', ['harvested', 'in_transit', 'delivered']);
            if (inventoryError) {
                logger_1.logger.error('Get inventory analytics error:', inventoryError);
                res.status(500).json({
                    success: false,
            ...
            const analytics = this.calculateInventoryAnalytics(inventory || [], incomingRes.data || [], outgoingRes.data || []);
            res.status(200).json({
                success: true,
                data: analytics
            });
            ...
            logger_1.logger.error('Get inventory analytics error:', error);
            res.status(500).json({
                success: false,
                message: 'Failed to retrieve inventory analytics'
            });
    ...
    extractDistributorSpecialties(batches) {
        const productTypes = new Set(batches.map(batch => batch.product_name?.toLowerCase()).filter(Boolean));
        return Array.from(productTypes);
    }
    calculateInventoryAnalytics(inventory, incoming, outgoing) {
        const totalValue = inventory.reduce((sum, batch) => sum + (parseFloat(batch.price_per_unit || '0') * parseFloat(batch.quantity || '0')), 0);
        const productBreakdown = inventory.reduce((acc, batch) => {
        ...
        const statusBreakdown = inventory.reduce((acc, batch) => {
        ...
        return {
            ...
            product_breakdown: Object.entries(productBreakdown).map(([name, data]) => ({
                product_name: name,
                ...data
            })),
...
exports.distributorController = new DistributorController();
