...
exports.geoController = exports.GeoController = void 0;
const supabase_1 = require("../config/supabase");
const logger_1 = require("../utils/logger");
class GeoController {
    // Add geolocation to batch
    async addBatchGeolocation(req, res) {
        try {
            ...
            const { error: updateError } = await supabase_1.supabaseAdmin
                ...
                .eq('id', batch.id);
            if (updateError) {
                logger_1.logger.error('Add geolocation error:', updateError);
                res.status(500).json({
                    success: false,
            ...
            logger_1.logger.info(`Geolocation updated for batch ${batchId} by user ${userId}`);
        }
        catch (error) {
            logger_1.logger.error('Add batch geolocation error:', error);
            res.status(500).json({
                success: false,
                message: 'Failed to add geolocation'
            });
    ...
    async getBatchLocationHistory(req, res) {
        try {
            const { batchId } = req.params;
            ...
            logger_1.logger.error('Get batch location history error:', error);
            res.status(500).json({
                success: false,
    ...
    async getBatchesByLocation(req, res) {
        try {
            ...
            const { data: batches, error } = await supabase_1.supabaseAdmin
                .from('produce_batches')
                .select(`
          id,
          batch_id,
        ...
        `)
            ...
            if (error) {
                logger_1.logger.error('Get batches by location error:', error);
                res.status(500).json({
                    success: false,
            ...
            const batchesInRadius = batches?.filter(batch => {
                ...
                ...batch,
                distance: this.calculateDistance(lat, lng, batch.metadata.geolocation.latitude, batch.metadata.geolocation.longitude).toFixed(2)
            })) || [];
            // Sort by distance
    ...
    async getUserLocationAnalytics(req, res) {
        try {
            const userId = req.user?.userId;
            if (!userId) {
                res.status(401).json({
            ...
            const { data: batches, error } = await supabase_1.supabaseAdmin
                ...
                .not('metadata->geolocation', 'is', null);
            if (error) {
                logger_1.logger.error('Get user location analytics error:', error);
                res.status(500).json({
                    success: false,
    ...
    generateLocationSummary(batches) {
        const regionCounts = {};
        const coordinates = [];
...
exports.geoController = new GeoController();
